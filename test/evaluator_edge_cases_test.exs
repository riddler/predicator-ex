defmodule EvaluatorEdgeCasesTest do
  ...
  describe "evaluator edge cases" do
    test "evaluator with instruction pointer beyond bounds finishes normally" do
      ...
      evaluator = %Evaluator{
        instructions: [["lit", 42]],
        instruction_pointer: 5,  # Beyond array bounds
        stack: [],
        context: %{}
      }
    ...
    test "comparison with insufficient stack values" do
      instructions = [["lit", 42], ["compare", "GT"]]  # Missing second value
    ...
    test "membership with non-list values" do
      ...
      assert {:error, msg} = Evaluator.evaluate([["lit", "not_a_list"], ["lit", 1], ["contains"]], %{})
    ...
    test "unknown instruction type" do
      ...
      assert {:error, msg} = Evaluator.evaluate([["compare"]], %{})  # Missing operator
      assert msg =~ "Unknown instruction:"
      
      # Instructions with wrong number of params
      assert {:error, msg} = Evaluator.evaluate([["lit"]], %{})  # Missing value
      assert msg =~ "Unknown instruction:"
    end
  end
  ...
  describe "evaluator state management" do
    test "step on finished evaluator" do
      evaluator = %Evaluator{
        instructions: [["lit", 42]],
        instruction_pointer: 1,  # Past end of instructions
        stack: [42],
        context: %{}
      }
