defmodule ParserCoverageTest do
  ...
  describe "parser edge cases for coverage" do
    ...
    test "empty token list parsing" do
      ...
      assert msg =~ "Expected number, string, boolean, date, datetime, identifier, function call, list, or '(' but found end of input"
    end
    
    test "unexpected tokens in primary expressions" do
      # Test various invalid token positions
      assert {:error, msg} = evaluate("AND true", %{})
      assert msg =~ "Expected number, string, boolean, date, datetime, identifier, function call, list, or '(' but found 'AND'"
      
      assert {:error, msg} = evaluate("OR false", %{})  
      assert msg =~ "Expected number, string, boolean, date, datetime, identifier, function call, list, or '(' but found 'OR'"
    ...
    test "invalid tokens in various contexts" do
      ...
      assert {:error, msg} = evaluate("[1 2]", %{})  # Missing comma
  ...
  describe "lexer edge cases for coverage" do
    ...
    test "invalid number formats" do
      ...
      {:ok, tokens} = Lexer.tokenize("00123")  # Should still work
