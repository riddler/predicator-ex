...
- [ ] New doctests run and pass (`mix test --only doctest` covered by the gate)
- [ ] Coverage for `Predicator.Instructions` stays above the 90% minimum in
      `coveralls.json`, with both `in_isa?/2` branches covered
- [ ] `mix corpus.generate --check` reports no drift (this phase changes no
      generated bytes)
