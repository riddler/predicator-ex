...
Shipped from this repo, language-neutral. Amended during implementation: the
corpus is **not** in the hex package files list. Nothing an application does at
runtime reads it, and the audience that does - sibling implementers - works
from a git checkout, so it is plainly visible on GitHub without putting
hundreds of test cases into every consumer's deps directory.
